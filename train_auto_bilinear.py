...
from embedding import Bilinear, Linear
...
def train(X_train, Y_train, X_val, Y_val, param):
    ...
    model = Linear(in2_features=dim_Y, bias=True)
    if GPU_AVAILABLE:
        model.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp", help="exp", type=int, default=1)
    parser.add_argument("--weights_path", help="weights_path", type=str)
    ...
    if ".word2vec" in args.vectors:
        vec_type = "word2vec"
    else:
        raise IOError("not a valid vectors file")

    SUFFIX = "auto_{}_{}_learning_rate_{}_batch_size_{}_epochs_{}_{}".format(args.exp, vec_type, args.learning_rate, args.batch_size, args.epochs, args.loss)
    ...
    for set_ in [k for k in anno.keys() if k != "tags"]:
        ...
        if args.exp == 1:
            ...
            for i, imid in tqdm(enumerate(imid_list), total=len(imid_list)):
                ...
                y = normalize_rows(np.array(y, dtype=np.float32))
                y = [t.reshape(1, -1) for t in y]
                y = feat_extractor.predict(kind="img_text", x=x, y=y)
                Y[set_][i] = y
